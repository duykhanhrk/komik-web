...
function ComicDetailMNPage() {
  ...
  const update = useMutation({
    mutationFn: () => ComicMNService.activeAsync(parseInt(comic_id || ''), !query.data.comic.active),
    ...
    <Page.Container>
      <Page.Content style={{flex: 0, flexDirection: 'row', position: 'sticky', top: 0, marginTop: -8, paddingTop: 8, paddingBottom: 8, backgroundColor: theme.colors.background}}>
        ...
        <View horizontal gap={8}>
          ...
          >
            <Icon icon={query.data.comic.active ? 'mingcute:eye-fill' : 'mingcute:eye-line'} style={{color: theme.colors.blue, height: 20, width: 20}}/>
            <Text style={{color: query.data.comic.active ? theme.colors.blue : theme.colors.foreground}}>Công khai</Text>
          </Button>
          <Button
            variant="primary"
        ...
        {tab === 'info' && (
          <>
            <Card shadowEffect animation="slideLeftIn">
              ...
              <ImageSection _data={query.data.comic} onSaveSuccess={query.refetch}/>
            </Card>
            <Card shadowEffect animation="slideRightIn">
              <InfoSection _data={query.data.comic} onSaveSuccess={query.refetch}/>
            </Card>
          </>
        )}
