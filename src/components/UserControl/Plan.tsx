...
function History() {
  ...
  const purchases = useMemo(() => query.data?.pages.flatMap(page => page.data), [query.data]);
      ...
      <View horizontal gap={8} style={{alignItems: 'center'}}>
        ...
        <View gap={4}>
          ...
          <Text variant="small" style={{color: theme.colors.tertiaryForeground}}>
            {query.data?.current_plan
              ? <>Cảm ơn bạn đã sử dụng dịch vụ của chúng tôi. Gói hiện tại của bạn có giá trị sử dụng đến <b>{moment(query.data?.current_plan.expires_at).format('DD-MM-YYYY HH:mm:ss')}</b></>
              : <>Bạn đã sẵn sàng bước chân vào cuộc hành trình mới đầy phấn khích với gói đăng ký độc đáo của chúng tôi?</>
            }
          </Text>
        </View>
      </View>
      { query.data?.current_plan ? null
        :
        <Button
          variant="primary"
          style={{columnGap: 8}}
